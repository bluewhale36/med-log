...
public class MedIntakeRecordService {
    ...
    public Optional<MedIntakeRecordDayViewDTO> updateRecordDayViewCache(AppUserUuid appUserUuid, LocalDate referenceDate) {
        log.info("Updating MedIntakeRecordDayViewCache for AppUserUuid={}, referenceDate={}", appUserUuid, referenceDate);

        /*
            1. AppUserUuid 에 해당하는 AppUserId 조회
         */
        Long appUserId = appUserIdentifierConvertService.getIdByUuid(appUserUuid);
        log.info("Found AppUserId: {} for AppUserUuid: {}", appUserId, appUserUuid);
        ...
        log.info("Found {} snapshots for AppUserId: {} on ReferenceDate: {}",
                snapshotList.size(), appUserId, referenceDate);
        ...
        if (!medIdList.isEmpty()) {
            ...
            for (MedViewProjection mewProjection : medProjectionList) {
                ...
                for (DoseTimeCount doseTimeCount : mewProjection.getDoseFrequency().getDoseFrequencyDetail().getDoseTimeCountList()) {
                    ...
                    // 기준 일자 + 투약 시간 -> 기준 일시
                    LocalDateTime referenceDateTime = LocalDateTime.of(referenceDate, doseTimeCount.getDoseTime());
                    ...
                    if (record != null) {
                        ...
                        viewItemTypeRecordDTOList.add(
                                MedIntakeRecordDayViewDTO.ViewItemTypeRecordDTO.of(
                                        record,
                                        LocalDateTime.of(
                                                referenceDate, doseTimeCount.getDoseTime()
                                        )
                                )
                        );
                        log.info(
